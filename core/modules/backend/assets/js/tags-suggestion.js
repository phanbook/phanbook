...
function updateTags(){
    ...
    for(var i = 0;i < window.list_tag.length; i++){
        $('<span class="post-tag">'+window.list_tag[i]+'<span class="fa fa-times delete-tag" title="remove this tag"></span></span>').insertAfter("#tag-input");
    }
...
$(document).ready(function() {
    ...
    if ($('#tags').val()) {
        ...
        tagArray.forEach(function (element, index){
            var val= '<span class="post-tag">' + element + '<span title="remove this tag" class="fa fa-times delete-tag"></span></span>';
